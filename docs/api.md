...
| `params[indexable]` | Object |  | |
| `params[indexable][text]` | String |  | A string of indexable information, which should only be provided for files which are either only partially indexable (such as compressed or geo files) or non-indexable files (such as video files) |
| `params[indexable][hash]` | String |  | The SHA-2 hash of the indexable text contents (SHA-512, thus 128 Chars). |
